#' GoogleGeoAPI: A package for laboratory work 4 of the Advanced R Programming
#' course.
#'
#' The package provides functions for (reverse) geocoding. (Reverse) geocoding
#' will give you the coordinates for a given address (the addres for given
#' coordinates).
#'
#' {\code{LinregClass}}:\itemize{
#' \item{\code{\link{adrs}} returns an S3 class object of type \code{adrs}.}
#' \item{\code{\link{coord}} returns an S3 class object of type \code{coord}.}
#' \item{\code{\link{geocode}} returns the result of (reverse) geocoding.}
#' }
#'
#' @docType package
#' @name GoogleGeoAPI
NULL
